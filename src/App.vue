...
<template>
  ...
  <div>
    <h2 class="text-2xl" v-if="books.length == 0">無代辦事項</h2>
    <ul v-else>
      <li class="text-2xl flex items-center gap-2" v-for="book in books">
        <input type="checkbox" :checked="book.completed" @change="toggleComplete(book)">
        <span :class="{ 'completed': book.completed }">{{ book.title }}</span>
